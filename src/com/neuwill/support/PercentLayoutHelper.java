 ...
 * <p/>
 * <p>
 * This class collects utility methods that are involved in extracting
 * percentage based dimension attributes and applying them to ViewGroup's
 * children. If you would like to implement a layout that supports percentage
 * based dimensions, you need to take several steps:
 * <p/>
 * <ol>
 * <li>You need a {@link ViewGroup.LayoutParams} subclass in your ViewGroup that
 * implements
 * {@link com.zhy.android.percent.support.PercentLayoutHelper.PercentLayoutParams}
 * .
 * <li>In your {@code LayoutParams(Context c, AttributeSet attrs)} constructor
 * create an instance of {@link PercentLayoutHelper.PercentLayoutInfo} by
 * calling
 * {@link PercentLayoutHelper#getPercentLayoutInfo(Context, AttributeSet)}.
 * Return this object from
 * {@code public PercentLayoutHelper.PercentLayoutInfo getPercentLayoutInfo()}
 * method that you implemented for
 * {@link com.zhy.android.percent.support.PercentLayoutHelper.PercentLayoutParams}
 * interface.
 * <li>Override
 * {@link ViewGroup.LayoutParams#setBaseAttributes(TypedArray, int, int)} with a
 * single line implementation
 * {@code PercentLayoutHelper.fetchWidthAndHeight(this, a,
 * widthAttr, heightAttr);}
 * <li>In your ViewGroup override
 * {@link ViewGroup#generateLayoutParams(AttributeSet)} to return your
 * LayoutParams.
 * <li>In your {@link ViewGroup#onMeasure(int, int)} override, you need to
 ...
 * </pre>
 * 
 * <li>In your {@link ViewGroup#onLayout(boolean, int, int, int, int)} override,
 * you need to implement following pattern:
 * 
 * <pre class="prettyprint">
...
public class PercentLayoutHelper {
	 ...
	 * calculated from percentage values.
	 *
	...
	public void adjustChildren(int widthMeasureSpec, int heightMeasureSpec) {
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			Log.d(TAG, "adjustChildren: " + mHost + " widthMeasureSpec: " + View.MeasureSpec.toString(widthMeasureSpec)
	 ...
	 * changed for percentage values. Calling this method only makes sense if
	...
	/**
	 * Iterates over children and checks if any of them would like to get more
	 * space than it received through the percentage dimension.
	 * <p/>
	 * If you are building a layout that supports percentage dimensions you are
	 * encouraged to take advantage of this method. The developer should be able
	 * to specify that a child should be remeasured by adding normal dimension
	 * attribute with {@code wrap_content} value. For example he might specify
	 * child's attributes as {@code app:layout_widthPercent="60%p"} and
	 * {@code android:layout_width="wrap_content"}. In this case if the child
	 * receives too little space, it will be remeasured with width set to
	 * {@code WRAP_CONTENT}.
	 *
	 * @return True if the measure phase needs to be rerun because one of the
	 *         children would like to receive more space.
	 */
	...
	public static class PercentLayoutInfo {
		...
		public String toString() {
			return "PercentLayoutInfo{" + "widthPercent=" + widthPercent + ", heightPercent=" + heightPercent
					...
					+ ", textSizePercent=" + textSizePercent + ", maxWidthPercent=" + maxWidthPercent
					+ ", maxHeightPercent=" + maxHeightPercent + ", minWidthPercent=" + minWidthPercent
					+ ", minHeightPercent=" + minHeightPercent + ", paddingLeftPercent=" + paddingLeftPercent
					+ ", paddingRightPercent=" + paddingRightPercent + ", paddingTopPercent=" + paddingTopPercent
					+ ", paddingBottomPercent=" + paddingBottomPercent + ", mPreservedParams=" + mPreservedParams + '}';
		}
		 ...
		 * called {@link PercentLayoutHelper.PercentLayoutInfo#fillLayoutParams}
		 * .
		 */
